...
- **Project health pre-screen (B)**: before scoring the issue, evaluate the repository:
  ...
  - Check for community health files: `curl -sL https://raw.githubusercontent.com/<owner>/<repo>/main/CODE_OF_CONECT.md | head -5` and `https://raw.githubusercontent.com/<owner>/<repo>/main/SECURITY.md | head -5`. If `/main/` fails, try `/master/`.
